...
export default function ChatPage({ username = 'Guest' }) {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: `👋 Hi ${username}, how can I assist your construction project today?` },
  ]);
    ...
    <div className="max-w-2xl mx-auto mt-8 p-4">
      ...
      <div className="h-96 overflow-y-auto border border-gray-300 rounded-md p-4 bg-gray-50 space-y-2">
        {messages.map((msg, index) => (
          ...
          >
            <div className={`px-3 py-2 rounded-lg shadow-sm max-w-xs ${msg.role === 'user' ? 'bg-blue-200' : 'bg-gray-200'}`}>
              {msg.content}
            </div>
          </div>
        ))}
